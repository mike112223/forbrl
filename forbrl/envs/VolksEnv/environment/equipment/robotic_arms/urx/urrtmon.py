"""
Module for implementing a UR controller real-time monitor over socket port
30003.
http://support.universal-robots.com/Technical/RealTimeClientInterface
Note: The packet length given in the web-page is 740. What is actually
received from the controller is 692. It is assumed that the motor currents,
the last group of 48 bytes, are not send. Originally Written by Morten Lind
"""
import logging
import socket
import struct
...
__author__ = "Morten Lind, Olivier Roulet-Dubonnet * 95% + Tianhe Wang * 5%"
...
class URRTMonitor(threading.Thread):
    ...
    def __recv_bytes(self, n_bytes):
        ...
        while len(pkg) < n_bytes:
            pkg += self._rtSock.recv(n_bytes - len(pkg))
            if recv_time == 0:
                recv_time = time.time()
        self.__recvTime = recv_time
        return pkg
    ...
    def tcf_pose(self, wait=False, timestamp=False, ctrl_timestamp=False):
        """ Return the tool pose values."""
        if wait:
            self.wait()
        with self._dataAccess:
            tcf = self._tcp
            if ctrl_timestamp or timestamp:
                ret = [tcf]
                if timestamp:
    ...
    def __recv_rt_data(self):
        ...
        with self._dataAccess:
            ...
            # it seems that packet often arrives packed as two... maybe
            # TCP_NODELAY is not set on UR controller?? if (self._timestamp -
            # self._last_ts) > 0.010: self.logger.warning("Error the we did
            # not receive a packet for {}s ".format( self._timestamp -
            # self._last_ts)) self._last_ts = self._timestamp
            self._ctrlTimestamp = np.array(unp[0])
            if self._last_ctrl_ts != 0 and (
                    self._ctrlTimestamp -
                    self._last_ctrl_ts) > 0.010:
                self.logger.warning(
                    "Error the controller failed to send us a packet: time "
                    "since last packet %s s ",
            ...
            if self._csys:
                with self._csys_lock:
                    # might be a good idea to remove dependency on m3d
            ...
            with self._buffer_lock:
                self._buffer.append(
                    (self._timestamp,
                     self._ctrlTimestamp,
                     self._tcp,
                     self._qActual))  # FIXME use named arrays of allow to
                # configure what data to buffer
