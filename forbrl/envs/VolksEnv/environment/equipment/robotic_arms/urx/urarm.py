...
class URArm(URRobot):
    """
    Generic Python interface to an industrial robot. Compared to the URRobot
    class, this class adds the possibility to work directly with matrices and
    includes support for setting a reference coordinate system
    """
    logger = logging.getLogger(__name__)
    reconnect_times = 50

    def __init__(self, host, use_rt=False, tcp=None, csys=None):
        for i in range(self.reconnect_times):
            try:
                ...
                if not self.is_running():
                    ...
                    self.logger.debug(f"Successfully connected after {i} "
                                      f"attempts")
                    break
            except:
                time.sleep(2)
            ...
            msg = (f"Tried {self.reconnect_times} but still failed to connect "
                   f"to the UR arm")
        ...
        if csys is None:
            self.set_csys(m3d.Transform())
        else:
            self.set_csys(m3d.Transform(csys))
    ...
    def set_orientation(self, orient, acc=0.01, vel=0.01, wait=True,
                        threshold=None):
        """
        set tool orientation using a orientation matrix from math3d
    ...
    def set_pose(self, trans, acc=0.01, vel=0.01, wait=True,
                 command="movel", threshold=None, qnear=None):
        """
        move tcp to point and orientation defined by a transformation UR
        robots have several move commands, by default movel is used but it
        can be changed using the command argument
        """
        self.logger.debug("Setting pose to %s", trans.pose_vector)
        t = self.csys * trans
        pose = URRobot.movex(self, command, t.pose_vector, acc=acc, vel=vel,
                             wait=wait, threshold=threshold, qnear=qnear)
        if pose is not None:
            return self.csys.inverse * m3d.Transform(pose)

    def add_pose_base(self, trans, acc=0.01, vel=0.01, wait=True,
                      command="movel", threshold=None, qnear=None):
        """add transform expressed in base coordinate"""
        pose = self.get_pose()
        return self.set_pose(trans * pose, acc, vel, wait=wait,
                             command=command, threshold=threshold, qnear=qnear)

    def add_pose_tool(self, trans, acc=0.01, vel=0.01, wait=True,
                      command="movel", threshold=None):
    ...
    def movex(self, command, pose, acc=0.01, vel=0.01, wait=True,
    ...
    def movexs(self, command, pose_list, acc=0.01, vel=0.01, radius=0.01,
        ...
        return URRobot.movexs(self, command, new_poses, acc, vel, radius,
    ...
    def new_csys_from_xpy(self):
        """
        Restores and returns new coordinate system calculated from three
        points: X, Origin, Y
        ...
        print("A new coordinate system will be defined from the next three "
              "points\nFirs point is X, second Origin, third Y\nSet it as a "
              "new reference by calling myrobot.set_csys(new_csys)")
        ...
        pose = URRobot.getl(self)
        print("Introduced point defining Origin: {}".format(pose[:3]))
