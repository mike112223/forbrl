...
class URRobot:
    ...
    Programs are send to port 3002 data is read from secondary interface(
    10Hz?) and real-time interface(125Hz) (called Matlab interface in
    documentation).
    Since parsing the RT interface uses som CPU, and does not support all robots
    versions, it is disabled by default The RT interfaces is only used for
    the get_force related methods
    Rmq: A program sent to the robot will be executed immediately and any
    running program will be stopped
    """
    ...
    def __init__(self, host, use_rt=False):
        ...
        self.secmon.wait()  # make sure we get data from robot before
        # letting clients access our methods
        self.logger.debug(f"Initialized {self.__class__.__name__}:\n"
                          f" {self.__repr__()}")

    def __repr__(self):
        return "Robot Object (IP=%s, state=%s)\n" % (
            self.host, self.secmon.get_all_data()["RobotModeData"])
    ...
    def send_program(self, prog):
        ...
        the program is executed immediately and any running
        program is interrupted
        """
        self.logger.debug(f"Sending program: {prog}")
    ...
    def _wait_for_move(self, target, threshold=None, timeout=5, joints=False):
        """
        wait for a move to complete. Unfortunately there is no good way to
        know when a move has finished so for every received data from robot
        we compute a dist equivalent and when it is lower than 'threshold'
        we return. if threshold is not reached within timeout, an exception
        is raised
        """
        self.logger.debug(
        ...
        while True:
            ...
            self.logger.debug("distance to target is: %s, target dist is %s",
                              dist, threshold)
            if not self.secmon.is_program_running():
                ...
                if count > timeout * 10:
                    raise RobotException(
                        "Goal not reached but no program has been running "
                        "for {} seconds. dist is {}, threshold is {}, "
                        "target is {}, current pose is {}".format(
                            timeout, dist, threshold, target,
                            URRobot.getl(self)))
            else:
                count = 0
    ...
    def _get_lin_dist(self, target):
        # FIXME: we have an issue here, it seems sometimes the axis angle
        #   received from robot
    ...
    def getj(self, wait=False):
        ...
        return [jts["q_actual0"], jts["q_actual1"], jts["q_actual2"],
                jts["q_actual3"], jts["q_actual4"], jts["q_actual5"]]
    ...
    def movej(self, joints, acc=0.1, vel=0.05, wait=True, relative=False,
        ...
        if relative:
            loc = self.getj()
            joints = [v + loc[i] for i, v in enumerate(joints)]
    ...
    def movelj(self, tpose, acc=0.01, vel=0.01, wait=True, relative=False,
               threshold=None, qnear=None):
        """Send a movel command to the robot. See URScript documentation."""
        return self.movex("movelj", tpose, acc=acc, vel=vel, wait=wait,
                          relative=relative, threshold=threshold, qnear=qnear)

    def movel(self, tpose, acc=0.01, vel=0.01, wait=True, relative=False,
              threshold=None):
        """Send a movel command to the robot. See URScript documentation."""
        return self.movex("movel", tpose, acc=acc, vel=vel, wait=wait,
    ...
    def movep(self, tpose, acc=0.01, vel=0.01, wait=True, relative=False,
        ...
        return self.movex("movep", tpose, acc=acc, vel=vel, wait=wait,
                          relative=relative, threshold=threshold)
    ...
    def servoc(self, tpose, acc=0.01, vel=0.01, wait=True, relative=False,
        ...
        return self.movex("servoc", tpose, acc=acc, vel=vel, wait=wait,
                          relative=relative, threshold=threshold)

    def _format_move(self, command, tpose, acc, vel, radius=0.0,
                     prefix="", qnear=None):
        tpose = [round(i, self.max_float_length) for i in tpose]
        extra_control = f"a={acc}, v={vel}, r={radius}"
        if command == 'movelj':
            inv_kin_prog = f"get_inverse_kin({prefix}{tpose})"
            if qnear is not None:
                inv_kin_prog = f"get_inverse_kin({prefix}{tpose}, " \
                               f"{qnear})"
            return f"movej({inv_kin_prog}, {extra_control})"
        return f"{command}({prefix}{tpose}, {extra_control})"

    def movex(self, command, tpose, acc=0.01, vel=0.01, wait=True,
              relative=False, threshold=None, qnear=None):
        """
        Send a move command to the robot. Since UR robot have several
        methods this one sends whatever is defined in 'command' string
        """
        if relative:
            loc = self.getl()
            tpose = [v + loc[i] for i, v in enumerate(tpose)]
        prog = self._format_move(command, tpose, acc, vel,
                                 prefix="p", qnear=qnear)
        self.send_program(prog)
        if wait:
    ...
    def movejs(self, joint_positions_list, acc=0.01, vel=0.01, radius=0.01,
        ...
        """
        return URRobot.movexs(self, "movej", joint_positions_list, acc, vel,
                              radius,
    ...
    def moveljs(self, pose_list, acc=0.01, vel=0.01, radius=0.01,
                wait=True, threshold=None):
        """
        Concatenate several movel commands and applies a blending radius
        pose_list is a list of pose.
        ...
        to robot make the robot stop
        """
        return self.movexs("movelj", pose_list, acc, vel, radius,
                           wait, threshold=threshold)
    ...
    def movexs(self, command, pose_list, acc=0.01, vel=0.01, radius=0.01,
        ...
        if len(radius) != len(pose_list):
            raise RobotException(
                ...
                + 'of numbers the same length as "pose_list"!')
        prefix = ''
        if command in ['movel', 'movec', 'movelj']:
            prefix = 'p'
        ...
        self.send_program(prog)
        if wait:
            if command.lower() in ['movel', 'movelj']:
    ...
    def close(self):
        ...
        self.logger.debug("Closing sockets to robot")
        self.secmon.close()
        if self.rtmon:
            self.rtmon.stop()
    ...
    def set_freedrive(self, val, timeout=60):
        ...
        if val:
            ...
            # This is a non-existant program, but running it will stop
            # freedrive
    ...
    def get_realtime_monitor(self):
        ...
        """
        if not self.rtmon:
            self.logger.debug("Opening real-time monitor socket")
            self.rtmon = urrtmon.URRTMonitor(self.host)  # some information is
            # only available on rt interface
            self.rtmon.start()
